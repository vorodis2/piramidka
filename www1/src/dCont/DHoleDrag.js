...
export class DHoleDrag extends DCont {
    constructor(dCont, _x, _y, _rect, _fun,_bool) {
        ...
        this.rect={x:0,y:0,width:200,height:100};
        this.rect={x:0,y:0,width:100,height:200};
        ...
        this._radius=10;
        // this._ratio=0;
        this._ratio=this.rect.width / this.rect.height;
        ...
        ////////////////////////////////////////////////////////////////////////////
        this.getPressedBtn = function() { // => Btn instance or null
            // LeftTop
            // RightTop
            // LeftBot
            // RightBot
        ...
        this.isRectVlazit = function() {
            return !(
                this.rect.x < 0 ||
                this.rect.y < 0 ||
                this.rect.width + this.rect.x > this._width ||
                this.rect.height + this.rect.y > this._height
            );
        };

        this.correctRect = function() {
            //trace('correctRect started', this.rect);
        
            var x=0;
            if(Math.round(this.rect.width / this._ratio)!=this.rect.height){
                this.rect.height=Math.round(this.rect.width / this._ratio)
            }
          ...
          /*  if (this.rect.x < 0) this.rect.x = 0;
            ...
            if (this.rect.width > this._width) {
                this.rect.width = this._width;
                this.rect.height = this.rect.width / this._ratio;
            }

            if (this.rect.height > this._height) {
                this.rect.height = this._height;
                this.rect.width = this.rect.height * this._ratio;
            }

            if (this.rect.height + this.rect.y > this._height) this.rect.y = this._height - this.rect.height;
            if (this.rect.width + this.rect.x > this._width) this.rect.x = this._width - this.rect.width;
            // 
            this.rect.width = Math.round(this.rect.width);
            this.rect.height = Math.round(this.rect.height);*/
            //trace('correctRect ended',this.idArr,this.tip, this.rect);
        };
        ...
        this.drag=function () {
            ...
            if(this._ratio == 0) {
                ...
                this.correctRect();
              /*  let vlazit = this.isRectVlazit();
                if (!vlazit) {
                    let pressedButton = this.getPressedBtn();
                    if ( !pressedButton ) {
                        this.correctRect();
                    } else {
                        // Scale by touched side to button
                    }
                }*/
            }
...
/*
            ...
            trace(point1.bx, point1.by, point2.bx, point2.by,point1,point2)
            ...
            this.drag()*/ 
            trace("point,point2")
            trace(point,point1,point2,point3)
            trace(self.point)


            pointXX.x=self.point.x+point3.x;
            pointXX.y=self.point.y+point3.y;


            trace(self.point.x, pointXX.x)
            if(point.x<pointXX.x){
                this.rect.x=point.x;
                this.rect.width=pointXX.x-point.x;
            }
           /* if(point.y<pointXX.y){
                this.rect.y=point.y;
                this.rect.height=pointXX.y-point.y;
            }*/
            trace(this.rect)
             //this.drag()
        }
    ...
    set ratio(value) {
        if (this._ratio != value) {
            if (value < 0) { this._ratio = 0; return; }
            if (value > 2) { this._ratio = 2; return; }
            this._ratio = value;
            this.drag();
        }
    }
